...
def mobilenetv3_small(inputs, num_classes, is_train=True):
    ...
    with tf.variable_scope('mobilenetv3_small'):
        ...
        net = mnv3_block(net, 5, 120, 48, 1, is_train, name='bneck5_1', h_swish=True, ratio=reduction_ratio, se=False) 
        net = mnv3_block(net, 5, 144, 48, 1, is_train, name='bneck5_2', h_swish=True, ratio=reduction_ratio, se=False)
        ...
        net = conv2d_hs(net, 576, is_train, name='conv7_1')
        net = global_avg(net,7)
        net = conv2d_hs(net, 1280, is_train, name='conv2d_NBN', bias=True)
        net = conv2d_hs(net, num_classes, is_train, name='logits',bias=True)
        logits = flatten(net)
